...
class ProposalLayer(KE.Layer):
    ...
    def call(self, inputs):
        ...
        proposals = utils.batch_slice([boxes, scores], nms,
                                      self.config.IMAGES_PER_GPU)
        proposals = tf.reshape(proposals, [-1, self.proposal_count, proposals.get_shape()[2]])
        return proposals
...
#         pooled = tf.reshape(pooled, (-1, boxesnum_per_img) + pooled.shape[2:])
#         pooled = tf.reshape(pooled, (-1, 1, pooled.shape[1]))
        return pooled
...
class DetectionLayer(KE.Layer):
    ...
    def call(self, inputs):
        ...
        db = tf.reshape(detections_batch,[-1, self.config.DETECTION_MAX_INSTANCES, 6])
...
#     rpn_class_logits = tf.reshape(rpn_class_logits, 
#                                   [-1, rpn_class_logits.get_shape()[1], rpn_class_logits.get_shape()[2],
#                                    rpn_class_logits.get_shape()[3]])
#     rpn_probs = tf.reshape(rpn_probs, [-1, rpn_probs.get_shape()[1], rpn_probs.get_shape()[2],rpn_probs.get_shape()[3]])
#     rpn_bbox = tf.reshape(rpn_bbox, [-1, rpn_bbox.get_shape()[1], rpn_bbox.get_shape()[2],rpn_bbox.get_shape()[3]])
...
#         rpn_rois = tf.reshape(rpn_rois, [-1, proposal_count, rpn_rois.get_shape()[2]])
        if mode == "training":
...
#         optimizer = tf.keras.optimizers.SGD()
    ...
    def train(self, train_dataset, val_dataset, learning_rate, epochs, layers,
        ...
        # Callbacks
        callbacks = [
            keras.callbacks.TensorBoard(log_dir="C:\\Users\\elishai.WISMAIN\\Documents\\tensorboard_logs\\", 
